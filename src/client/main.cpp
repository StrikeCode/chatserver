    ...
    else // 登录成功
    {
        ...
        if (responsejs.contains("offlinemsg"))
        {
            ...
            for (string &str : vec)
            {
                ...
                if (ONE_CHAT_MSG == js["msgid"].get<int>()) // 接收一对一聊天消息
                {
                    cout << js.dump() << endl;
...
void mainMenu(int clientfd)
{
    ...
    while (isMainMenuRunning)
    {
        ...
        {
            command = commandbuf.substr(0, idx); // 截取出命令名来访问函数表中指定函数
        }
        ...
        // 如果是 creategroup:groupname:groupdesc
        // 第二参数为groupname:groupdesc
...
void chat(int clientfd, string str)
{
    ...
    string buffer = js.dump();
    // cout << "chat(int clientfd, string str):buffer = " << buffer << endl;
